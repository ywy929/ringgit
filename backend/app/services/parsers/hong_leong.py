...
# Matches "01/04/2026" at the start of a line
TRANSACTION_LINE_RE = re.compile(r"^(\d{2}/\d{2}/\d{4})\s{2,}(.+)$")
AMOUNT_RE = re.compile(r"[\d,]+\.\d{2}")
PERIOD_RE = re.compile(r"Period:\s*\d{2} \w{3} \d{4}\s+to\s+(\d{2} \w{3} \d{4})")

# Column positions from the sample:
#   Trans Date (0), Description (14), Debit (RM) (48), Credit (RM) (63), Balance (RM) (78)
_COL_DR = 48
_COL_CR = 63
_COL_BAL = 78
...
class HongLeongParser(BaseParser):
    @property
    ...
    def can_parse(self, text: str) -> bool:
        upper = text.upper()
        return "HONG LEONG BANK" in upper
